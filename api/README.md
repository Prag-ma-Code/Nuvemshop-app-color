...
# Nuvemshop script id configured in the Partners Portal
SCRIPT_ID=
...
The API now also persists custom color mappings in `db.json` under the `customColors` collection. Those records are used by the new public route `GET /api/public/custom-colors?product_id=X` and by the embedded admin screen that saves variant-name-to-hex associations.
